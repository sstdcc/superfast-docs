  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Left: copyright + SSTD logo */}
          <div style={{ fontSize: 12.5, color: c.copy, display: 'flex', alignItems: 'center', gap: 12 }}>
            <a
              href="https://www.sstd.cc"
              target="_blank"
              rel="noreferrer"
              style={{ display: 'flex', alignItems: 'center', textDecoration: 'none', opacity: isDark ? 0.7 : 0.5, transition: 'opacity 0.15s' }}
              onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.opacity = '1'; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.opacity = isDark ? '0.7' : '0.5'; }}
            >
              <img
                src={`${siteConfig.baseUrl}img/sstd-logo.png`}
                alt="SSTD"
                height={22}
                style={{ filter: isDark ? 'none' : 'invert(1)', display: 'block' }}
              />
            </a>
            <span style={{ color: c.dot }}>·</span>
            <span>© {YEAR} SSTD. All rights reserved.</span>
            <span style={{ color: c.dot }}>·</span>
            <span>
              Built with{' '}
              <a
          ...
          <div style={{ display: 'flex', gap: 20 }}>
            {[
              { label: 'Privacy Policy', href: 'https://www.sstd.cc/privacy' },
              { label: 'Terms of Service', href: 'https://www.sstd.cc/terms' },
              { label: 'Support', href: 'https://github.com/sstdcc/superfast/issues' },
            ].map(({ label, href }) => (
              <a
                key={label}
                href={href}
